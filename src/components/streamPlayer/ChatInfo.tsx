...
const ChatInfo = ({isDelayed,isFollowersOnly}:ChatInfoProps) => {
    ...
    const hint = useMemo(()=>{
        ...
        if(!isFollowersOnly && isDelayed) return 'Messages are delayed by 3 seconds'
        if(isFollowersOnly && isDelayed) return 'Only followers can chat. Messages are delayed by 3 seconds'
        return "";
    ...
    const label = useMemo(()=>{
        ...
        if(isFollowersOnly && isDelayed) return 'Only followers and slow mode'
        return "";
    },[isDelayed,isFollowersOnly])
